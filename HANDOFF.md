...
4. **앱에 다시 심기** — 부품이나 CSS를 고쳤으면 이 저장소를 쓰는 앱에도 옮깁니다.

   ```bash
   npm run sync:app ../cogniterm-vue            # 심기
   npm run sync:app ../cogniterm-vue -- --check # 어긋났는지만 보기
   ```

   앱 쪽에서는 `npm run ds:check` · `npm run ds:sync` 로 부릅니다 (앱 루트의 `ds.app.json`).
   2-2절의 복사 절차를 그대로 코드로 옮긴 것이고, 토큰 블록 제거·`meta.ts` 제외·
   유령 파일 삭제까지 합니다.

   > **`live/`와 같은 사고가 앱에서도 났습니다.** front-ux는 2026-08-19판을 08-28까지
   > 들고 있었고, 커밋 제목은 "원본과 동기화"였지만 `version.ts` 스탬프는 08-19였습니다.
   > **스탬프는 사본이 맞는지 말해 주지 않습니다** — `--check`는 파일 내용을 봅니다.
...
- ~~**`--cat-*`가 콘솔과 값이 다릅니다**~~ — 2026-08-28 해결. 범주 팔레트를 차트에서
  갈라내고(`--chart-*`가 자기 값을 갖습니다) `--cat-1~8`을 콘솔 값으로 맞췄습니다.
  `.badge.{violet,teal,pink}`은 `.badge.cat-{3,2,5}`로 개명했습니다 — 색 이름은 값이
  바뀌는 순간 거짓말이 됩니다. CHANGELOG 참고.
...
|---|---|---|---|
| 버튼 | `.btn` · `.btn-primary` · `.btn-danger` · `.btn-sm` | `DsButton variant/size` | `btn primary` · `btn danger` · `btn sm` |
| 배지 | `.badge` + `success` `warning` `danger` `info` `outline` `solid` | `DsBadge` | `.pill--ok/err/warn/info/line/dim/caut/ink` |
| 입력칸 | `.input` (textarea · select 도) | `DsInput`(래퍼 `.field` + `.input`) | `.field` · 이용자 `.hfield` |
| 드롭다운 트리거 | `.selbtn` (`.is-on` `.is-open` `.val` `.x`) | `DsSelect` | — (콘솔 어댑터 `.selbtn.ds-sel` 은 ui.css) |
| 검색칸 | `.searchwrap .input` | `DsSearchField` | — |
| 필터 버튼 · 패널 | `.fbtn` `.fbadge` · `.filpanel .fp-*` | 앱 공용 `FilterPanel.vue` | — |
