...
- **`--cat-2` · `--cat-3`이 콘솔과 값이 다릅니다** (2026-08-28 확인). 여기는
  틸 `#16a3a3` · 바이올렛 `#8b5cf6`인데 콘솔(llmops-console-ux)은 주황 `#eb6834` ·
  초록 `#1baf7a`입니다. 토큰 기준은 **콘솔이 원본**이므로 여기가 따라가야 하는데,
  `.badge.teal`이 `--cat-2`를 쓰면서 이름이 `teal`이라 **개명이 같이 따라옵니다**.
  그래서 이번(7 · 8 추가)에는 손대지 않았습니다.
  ⚠ 그 여파로 지금 팔레트에 **비슷한 색이 두 쌍** 보입니다 — 2(틸)와 7(청록),
  3(바이올렛)과 8(보라). 2 · 3을 콘솔 값으로 맞추면 자연히 풀립니다.
  같이 볼 일: `.badge.teal` → 무슨 이름으로 바꿀지.
