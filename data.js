...
/* ══════════════ DATA INPUT ══════════════ */
{
  ...
  props: [
    ...
    ['size', `'sm' | 'default'`, `'default'`, '컨트롤 스케일 — 기본 32 / md 36 / lg 40 (sm은 32의 다른 이름).'],
  ],
  slots: [],
...
},
{
  ...
  reason: { ko: '알림의 생김새를 한 곳에 둡니다 — Snackbar가 이걸 그대로 씁니다. 면은 뒤집습니다(라이트=어두운 면, 다크=밝은 면): 화면 위에 잠깐 떠 있다 사라지는 물건이라 본문과 같은 면이면 눈에 들어오지 않습니다.',
...
/* ══════════════ OVERLAY ══════════════ */
{
  ...
  guidelines: [
    ...
    ['해야 할 것', '확인 창은 바깥을 눌러도 닫히게 둡니다 — 닫히는 결과가 [취소]와 같아 막을 이유가 없습니다.'],
    ['하지 말 것', '쓰다 만 입력이 있다고 나가는 길에 확인을 끼우지 않습니다. 창이 [저장]으로 받는 것이 없으면 그냥 닫습니다 — 자주 하는 일을 두 번 누르게 만들 뿐입니다.'],
...
},
{
  ...
  props: [
    ...
    ['checkbox', 'boolean', 'false', '다중 선택 피커에서 앞에 체크박스를 노출.'],
    ['disabled', 'boolean', 'false', '선택 불가 — 흐리게, 클릭 안 됨.'],
  ],
  events: [['select', '—', '행 클릭 (disabled면 발생 안 함).']],
  slots: [
...
},
{
  ...
  demos: [
    ...
    { key: 'sizes',    title: 'Sizes',    desc: '32 / 36 / 40px — 다른 입력 컨트롤과 같은 두 단계. 필터 바·툴바는 sm.' },
...
},
{
  ...
  origin: 'custom', vuetifyBase: null,
  summary: '여러 선택지 중 하나.',
  reason: { ko: 'VRadioGroup을 쓰다가 버렸습니다 — 선택 표시를 아이콘 글리프(CircleDot)로 그려 안쪽 점이 1.5px 선의 아주 작은 원으로 나왔고, 크기를 키워도 비율이 어긋났습니다. 원과 점을 CSS로 그려 어떤 크기에서도 점이 상자의 37.5%로 유지됩니다. 항목마다 부연 설명이 필요한 경우가 많아 hint도 구조에 포함시켰습니다.',
            en: 'Dropped VRadioGroup — the selected dot was an icon glyph rendered as a hairline circle and never scaled right. Circle and dot are drawn in CSS at a fixed ratio. Per-option hints are baked in.' },
  props: [
    ['modelValue', 'any', '—', 'v-model.'],
    ['label', 'string', '—', '그룹 라벨.'],
    ['items', '{ value, label, hint? }[]', '필수', '선택지.'],
    ['inline', 'boolean', 'false', '가로 배치.'],
    ['size', `'sm' | 'default' | 'lg'`, `'default'`, '원 14 / 16 / 20px.'],
    ['disabled', 'boolean', 'false', '그룹 전체 비활성. 항목별로는 items[].disabled.'],
  ],
  slots: [],
  demo: `<div class="ds-radio"><div class="ds-radio__group"><label class="ds-radio__row"><span class="ds-radio__mark"></span><span class="ds-radio__text">90일<span class="ds-radio__hint">기본값</span></span></label><label class="ds-radio__row"><span class="ds-radio__mark is-on"></span><span class="ds-radio__text">180일<span class="ds-radio__hint">현재 설정</span></span></label></div></div>`,
...
},
{
  ...
  slots: [],
  demo: `<div style="max-width:280px"><div style="display:flex;justify-content:space-between;font-size:13px;font-weight:500"><span>신뢰도 임계값</span><span style="font-family:var(--mono);color:var(--gray-11);font-weight:400">70%</span></div><div style="height:6px;background:var(--gray-5);border-radius:999px;margin-top:12px;position:relative"><div style="width:70%;height:100%;background:var(--brand);border-radius:999px"></div><i style="position:absolute;left:70%;top:-5px;width:16px;height:16px;box-sizing:border-box;border-radius:50%;background:var(--primary-foreground);border:1px solid var(--brand);margin-left:-11.2px;display:block"></i></div></div>`,
  vue: `<DsSlider v-model="threshold" label="신뢰도 임계값" suffix="%" />`,
  html: null,
  guidelines: [
    ['해야 할 것', '정확한 값이 중요하면 숫자 입력을 함께 제공합니다. 슬라이더는 감을 잡는 도구라 0.35 같은 값을 손으로 못 집습니다.'],
    ['하지 말 것', '손잡이를 브랜드 색으로 꽉 채우지 않습니다. 채워진 구간과 한 덩어리로 붙어 지금 어디를 잡고 있는지 안 보입니다 — 흰 면에 브랜드 테두리 한 겹이 그 구분을 만듭니다.'],
    ['하지 말 것', '네이티브 `input[type=range]` + `accent-color`로 대신하지 않습니다. 손잡이가 꽉 찬 원으로만 나오고 브라우저마다 다르게 생깁니다.'],
  ],
},
{
...
{
  id: 'calendar', name: 'Calendar', ko: '달력', category: 'input',
  origin: 'wrapped', vuetifyBase: 'VDatePicker',
  summary: '화면에 펼쳐 두는 달력 격자.',
  ...
  props: [
    ['modelValue', 'Date | Date[]', '—', 'v-model. mode에 따라 하나 또는 배열.'],
    ['mode', `'single' | 'multiple' | 'range'`, `'single'`, '하루 · 여러 날 · 기간.'],
    ['min', 'string | Date', '—', '고를 수 있는 가장 이른 날.'],
    ['max', 'string | Date', '—', '고를 수 있는 가장 늦은 날.'],
    ['disablePast', 'boolean', 'false', '지난 날짜 차단 — 예약처럼 앞날만 고를 때.'],
    ['disableFuture', 'boolean', 'false', '앞날 차단 — 감사 로그처럼 지난 기간만 고를 때.'],
  ],
  slots: [],
  demos: [
    { key: 'single',   title: 'Single',   desc: '하루 — 가장 흔한 형태.' },
    { key: 'range',    title: 'Range',    desc: '기간 — 시작과 종료 사이가 옅은 면으로 이어집니다.' },
    { key: 'multiple', title: 'Multiple', desc: '이어지지 않는 여러 날 — 휴무일 지정처럼.' },
    { key: 'bounded',  title: 'Bounded',  desc: '지난 날짜 차단. 고를 수 없는 날은 흐리게 남겨 격자를 유지합니다.' },
  ],
  thumb: `<div style="border:1px solid var(--border);border-radius:var(--r-xl);padding:10px;width:212px;background:var(--surface)"><div style="font-size:13px;font-weight:500;margin-bottom:8px">2026년 8월</div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;font-size:10.5px;color:var(--gray-11);text-align:center"><span>일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span>토</span></div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;font-size:11.5px;text-align:center;margin-top:4px"><span></span><span></span><span></span><span></span><span></span><span></span><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><span>10</span><span style="background:var(--brand);color:#fff;border-radius:6px">11</span><span>12</span><span>13</span><span>14</span><span>15</span></div></div>`,
  demo: `<div style="border:1px solid var(--border);border-radius:var(--r-xl);padding:10px;width:212px;background:var(--surface)"><div style="font-size:13px;font-weight:500;margin-bottom:8px">2026년 8월</div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;font-size:10.5px;color:var(--gray-11);text-align:center"><span>일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span>토</span></div><div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;font-size:11.5px;text-align:center;margin-top:4px"><span></span><span></span><span></span><span></span><span></span><span></span><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><span>10</span><span style="background:var(--brand);color:#fff;border-radius:6px">11</span><span>12</span><span>13</span><span>14</span><span>15</span></div></div>`,
  vue: `<DsCalendar v-model="range" mode="range" disable-future />`,
  html: null,
  guidelines: [
    ['해야 할 것', '늘 보여야 하는 달력에만 씁니다. 폼 안의 날짜 한 칸은 DatePicker입니다 — 달력이 폼 높이를 세 배로 만듭니다.'],
    ['해야 할 것', 'createVuetify에 locale ko와 date ko-KR을 넣습니다. 없으면 요일이 S M T W T F S로 남습니다.'],
    ['하지 말 것', '오늘을 선택과 같은 브랜드 면으로 칠하지 않습니다 — 고르지도 않은 날이 골라진 것처럼 보입니다.'],
  ],
},
{
  id: 'datepicker', name: 'DatePicker', ko: '날짜 선택', category: 'input',
  origin: 'wrapped', vuetifyBase: 'VMenu + VDatePicker',
  summary: '필드를 눌러 달력에서 날짜를 고릅니다.',
  reason: { ko: 'Input · Menu · Calendar를 조합만 합니다. 새로 그리는 시각 요소가 없어 셋 중 하나가 바뀌면 여기도 따라 바뀝니다.',
            en: 'Composes Input + Menu + Calendar; no new visuals, so it never drifts from them.' },
  props: [
    ['modelValue', 'Date | Date[]', '—', 'v-model. mode가 range면 [시작, 종료].'],
    ['mode', `'single' | 'range'`, `'single'`, '하루 · 기간.'],
    ['label', 'string', '—', '라벨.'],
    ['hint', 'string', '—', '보조 설명.'],
    ['error', 'string', '—', '에러 메시지. 있으면 에러 상태.'],
    ['placeholder', 'string', `'날짜 선택'`, '비었을 때 문구.'],
    ['size', `'sm' | 'default'`, `'default'`, '32 / 36 / 40px — 필터 바·툴바는 sm.'],
    ['clearable', 'boolean', 'false', '지우기 버튼.'],
    ['presets', '[string, number][]', '—', `기간 프리셋. 예: [['최근 7일', 7]].`],
    ['min · max · disablePast · disableFuture', '—', '—', 'Calendar와 같습니다.'],
  ],
  slots: [],
  demos: [
    { key: 'default',   title: 'Default',   desc: '기본 — 비었을 때는 무엇을 고르는 칸인지 문구로 알립니다.' },
    { key: 'range',     title: 'Range',     desc: '기간 — 고른 값은 "시작 ~ 종료"로 한 줄에 보입니다.' },
    { key: 'presets',   title: 'Presets',   desc: '기간 프리셋. 최근 7일을 두 번 클릭으로 만들지 않습니다.' },
    { key: 'sizes',     title: 'Sizes',     desc: '32 / 36 / 40px — 다른 입력 컨트롤과 같은 두 단계.' },
  ...
  disable-future clearable />`,
  html: null,
  guidelines: [
    ['해야 할 것', '기간 선택에는 프리셋을 답니다. "최근 30일"을 달력에서 두 번 클릭으로 찾게 하지 않습니다.'],
    ['해야 할 것', '고를 수 없는 범위는 min·max로 미리 막습니다. 고른 뒤에 에러로 알리지 않습니다.'],
    ['하지 말 것', '날짜를 직접 입력하게 하지 않습니다. 형식 검증과 오타 안내가 통째로 따라옵니다.'],
  ],
},
{
...
},
{
  ...
  summary: '떴다가 사라지는 알림.',
  reason: { ko: '"언제 뜨고 언제 사라지는가"만 담당합니다 — 화면 구석 배치·타이머·전환을 Vuetify가 처리합니다. 생김새는 DsToast를 그대로 렌더하므로 둘이 어긋나지 않습니다.',
...
},
{
  ...
  origin: 'custom', vuetifyBase: null,
  summary: '시간순 이력. 속이 빈 링과 이어 주는 선.',
  reason: { ko: '시간순 이력을 평범한 목록으로 만들면 "무엇이 먼저인지"가 읽히지 않습니다. VTimeline은 점이 면을 채운 원이라 링으로 바꿀 수 없고, 점 크기·선 두께·간격이 density 프리셋에 묶여 통제되지 않아 2026-08-28에 버렸습니다.',
            en: 'Chronology is lost when history is rendered as a plain list. Dropped VTimeline in 2026-08: its dot is a filled disc that cannot become a ring, and its sizing is locked to density presets.' },
  props: [
    ['items', 'TimelineItem[]', '필수', "{ id, time, datetime?, title, body?, variant? } — variant는 default·success·danger·brand"],
    ['ariaLabel', 'string', '—', '<ol>에 붙는 이름.'],
  ],
  slots: [
    ['title', '제목 줄을 직접 그릴 때. { item }을 받습니다.'],
    ['body', '본문 줄 — 두 값을 좌우로 벌리는 등 배치가 필요할 때.'],
    ['time', '시각 줄 — 상태 라벨을 함께 놓을 때.'],
  ],
  demo: `<ol class="ds-timeline"><li class="ds-timeline__item ds-timeline__item--danger"><p class="ds-timeline__title">삭제 시도 차단</p><div class="ds-timeline__body">권한 없음 — 법무 폴더</div><time class="ds-timeline__time">09:14</time></li><li class="ds-timeline__item ds-timeline__item--success"><p class="ds-timeline__title">분류 완료</p><div class="ds-timeline__body">문서 42건 · 12.4초</div><time class="ds-timeline__time">09:12</time></li><li class="ds-timeline__item"><p class="ds-timeline__title">수집 시작</p><time class="ds-timeline__time">09:11</time></li></ol>`,
  vue: `<DsTimeline :items="[
  { id: 1, time: '09:14', datetime: '2026-08-28T09:14',
    title: '삭제 시도 차단', body: '권한 없음 — 법무 폴더', variant: 'danger' },
]" />`,
  html: null,
  guidelines: [
    ['해야 할 것', '항목이 20개를 넘으면 날짜로 묶고 더보기를 제공합니다.'],
    ['해야 할 것', '글의 차례는 제목 → 본문 → 시각입니다. 먼저 읽혀야 하는 것은 "무엇이 있었나"입니다.'],
    ['하지 말 것', '성공·실패를 링 색만으로 가르지 않습니다 — 색각 이상에서 사라집니다. 글로도 말하세요.'],
    ['하지 말 것', 'VTimeline으로 되돌아가지 않습니다 — 점을 링으로 못 바꾸고 치수가 density에 묶입니다.'],
  ],
},
{
...
/* ══════════════ 확장 (Astryx 격차 보완) ══════════════ */
{
  ...
  props: [['href','string','필수','이동 주소.'],['external','boolean','false','새 탭. rel=noopener 자동.']],
  slots: [['default','링크 텍스트.'],['icon','뒤에 붙는 아이콘 (외부 표시 등).']],
  demo: `<span style="font-size:var(--text-md)">자세한 내용은 <a class="ds-link" href="#">감사 로그 문서</a>를 참고하세요.</span>`,
  vue: `<DsLink href="/docs/audit">감사 로그 문서</DsLink>
<DsLink href="https://…" external>외부 문서<template #icon>${'​'}</template></DsLink>`,
  html: `<a class="ds-link" href="/docs/audit">감사 로그 문서</a>`,
  guidelines: [['해야 할 것','링크는 이동, 버튼은 실행 — 동작을 실행하면 Button을 씁니다.'],['하지 말 것','"여기"를 링크 텍스트로 쓰지 않습니다. 목적지를 씁니다.']],
},
{
  id: 'kbd', name: 'Kbd', ko: '키 표기', category: 'content',
  origin: 'custom', vuetifyBase: null,
  summary: '키보드 키를 표기합니다.',
  reason: { ko: '단축키 안내의 시각을 고정합니다.', en: 'Fixes shortcut notation.' },
  props: [['keys','string[]','—','여러 키 조합. 없으면 슬롯 하나.']],
  slots: [['default','키 하나.']],
  demo: `<span style="display:inline-flex;gap:4px"><kbd class="kbd">⌘</kbd><kbd class="kbd">K</kbd></span>`,
  vue: `<DsKbd :keys="['⌘', 'K']" />
<DsKbd>Esc</DsKbd>`,
  html: `<kbd class="kbd">⌘</kbd><kbd class="kbd">K</kbd>`,
  guidelines: [['해야 할 것','기능 이름과 함께 씁니다 — "⌘K로 어디서든 검색".']],
},
{
  id: 'code', name: 'Code', ko: '코드', category: 'content',
  origin: 'custom', vuetifyBase: null,
  summary: '인라인 · 블록 코드.',
  reason: { ko: '고정폭·복사 버튼·tabular 숫자를 한 곳에 고정합니다.', en: 'Mono, copy button, tabular digits in one place.' },
  props: [['block','boolean','false','블록 형태 + 복사 버튼.'],['copyable','boolean','true','블록일 때 복사 버튼.']],
  slots: [['default','코드 내용.']],
  demo: `<div style="width:100%"><code class="ds-code">agent.run()</code>
  <div class="ds-codeblock" style="margin-top:10px"><pre><code>const result = await agent.run({
  source: 'drive',
})</code></pre></div></div>`,
  vue: `<DsCode>agent.run()</DsCode>
<DsCode block>const result = await agent.run()</DsCode>`,
  html: `<code class="ds-code">agent.run()</code>`,
...
},
{
  ...
  reason: { ko: '"7일 이내 상대, 이후 절대·고정폭" 규칙을 화면마다 다시 구현하지 않게 합니다.', en: 'Encodes the relative/absolute time rule once.' },
  props: [['value','Date | string | number','필수','시각.'],['mode',`'auto'|'absolute'|'relative'`,`'auto'`,'auto = 7일 기준 자동.']],
  slots: [],
  demo: `<div class="row" style="gap:18px"><time class="ds-time">2시간 전</time><time class="ds-time">2026-07-28 09:14</time></div>`,
  vue: `<DsTimestamp :value="log.createdAt" />
<DsTimestamp :value="log.createdAt" mode="absolute" />`,
  html: `<time class="ds-time" datetime="2026-08-03T09:14:00Z">2시간 전</time>`,
  guidelines: [['해야 할 것','감사·법적 기록은 mode="absolute"로 고정합니다.']],
},
{
  id: 'metalist', name: 'MetaList', ko: '메타 목록', category: 'content',
  origin: 'custom', vuetifyBase: null,
  summary: '키-값 정보 나열.',
  reason: { ko: '상세 패널의 라벨 폭·정렬이 화면마다 달라지는 것을 막습니다.', en: 'Fixes the key-value layout of detail panels.' },
  props: [['items','MetaItem[]','필수','{ label, value, mono? }']],
  slots: [['(label)','해당 라벨 값을 커스텀 렌더 (배지 등).']],
  demo: `<dl class="metalist" style="max-width:300px"><div class="metalist-row"><dt>수행자</dt><dd>Jiyong Kim</dd></div><div class="metalist-row"><dt>이벤트 ID</dt><dd class="mono">evt_8f3a2c91</dd></div><div class="metalist-row"><dt>IP</dt><dd class="mono">10.4.22.108</dd></div></dl>`,
  vue: `<DsMetaList :items="[
  { label: '수행자', value: 'Jiyong Kim' },
  { label: '이벤트 ID', value: 'evt_8f3a2c91', mono: true },
]" />`,
  html: `<dl class="metalist">
  <div class="metalist-row"><dt>수행자</dt><dd>Jiyong Kim</dd></div>
</dl>`,
  guidelines: [['해야 할 것','ID·IP·시각은 mono로.']],
},
{
  id: 'searchfield', name: 'SearchField', ko: '검색 입력', category: 'input',
  origin: 'custom', vuetifyBase: null,
  summary: '검색 전용 입력 — 돋보기 · 지우기 · 단축키 힌트 · 로딩.',
  reason: { ko: '검색창의 부속(아이콘·⌘K·클리어)이 화면마다 제각각이 되는 것을 막습니다.', en: 'One search affordance set, everywhere.' },
...
{
  id: 'systemmessage', name: 'SystemMessage', ko: '시스템 메시지', category: 'agent',
  origin: 'custom', vuetifyBase: null,
  summary: '대화 속 시스템 알림.',
  reason: { ko: '"대화가 요약되었습니다" 같은 안내는 참여자 발화가 아닙니다 — 중앙의 낮은 톤으로 구분합니다.', en: 'System notices are not utterances; centered low-key row.' },
  props: [['time','string','—','옆에 붙는 시각.']],
  slots: [['default','안내 문구.']],
  demo: `<div style="width:100%"><div class="sysmsg"><span class="sysmsg-text">이전 대화가 요약되었습니다</span><span class="sysmsg-time">09:14</span></div></div>`,
  vue: `<DsSystemMessage time="09:14">이전 대화가 요약되었습니다</DsSystemMessage>`,
  html: `<div class="sysmsg" role="status">이전 대화가 요약되었습니다</div>`,
...
{
  id: 'visuallyhidden', name: 'VisuallyHidden', ko: '시각 숨김', category: 'content',
  origin: 'custom', vuetifyBase: null,
  summary: '화면에는 없고 스크린리더에는 읽힘.',
  reason: { ko: '시각적으로 자명한 맥락을 보조기기에 전달할 표준 방법이 필요합니다.', en: 'Standard SR-only text.' },
  props: [],
  slots: [['default','읽힐 텍스트.']],
  demo: `<span style="font-size:var(--text-sm);color:var(--gray-10)">화면에는 아무것도 렌더되지 않습니다 — <code class="ds-code">.sr-only</code></span>`,
  vue: `<DsVisuallyHidden>검색 결과 12건</DsVisuallyHidden>`,
  html: `<span class="sr-only">검색 결과 12건</span>`,
  guidelines: [['해야 할 것','aria-label로 충분하면 그쪽을 먼저.']],
},
{
  id: 'numberinput', name: 'NumberInput', ko: '숫자 입력', category: 'input',
  origin: 'wrapped', vuetifyBase: 'VNumberInput',
  summary: '스텝 버튼이 있는 숫자 입력.',
  reason: { ko: '스텝 버튼, ↑↓ 키, min/max 클램프를 직접 만들면 IME·붙여넣기에서 깨집니다.', en: 'Steppers, arrow keys and clamping are easy to get wrong.' },
  props: [['modelValue','number','—','v-model.'],['min / max / step','number','— / — / 1','범위.'],['label / hint / error','string','—','필드 공통.'],['size',`'sm' | 'default'`,`'default'`,'컨트롤 스케일 — 기본 32 / md 36 / lg 40 (sm은 32의 다른 이름).']],
  slots: [],
  demo: `<div class="field" style="width:180px"><label>재시도 횟수</label><div class="input" style="display:flex;align-items:center;justify-content:space-between"><span>3</span><span style="display:flex;flex-direction:column;color:var(--gray-9);font-size:9px;line-height:1"><span>▲</span><span>▼</span></span></div></div>`,
  vue: `<DsNumberInput v-model="retries" label="재시도 횟수" :min="0" :max="10" />`,
  html: null,
  guidelines: [['해야 할 것','단위가 있으면 hint에 — "초", "건".']],
},
{
  id: 'combobox', name: 'Combobox', ko: '자유 태그 입력', category: 'input',
  origin: 'wrapped', vuetifyBase: 'VCombobox',
  summary: '목록에 없는 값도 입력하는 태그 입력.',
  reason: { ko: '자유 입력 + 제안 + 칩 관리 + 한글 조합을 함께 처리해야 합니다.', en: 'Free entry + suggestions + chips + IME.' },
  props: [['modelValue','string[]','[]','v-model — 칩 배열.'],['items','string[]','—','제안 목록.'],['label / hint / error','string','—','필드 공통.'],['size',`'sm' | 'default'`,`'default'`,'컨트롤 스케일 — 기본 32 / md 36 / lg 40 (sm은 32의 다른 이름).']],
  slots: [],
  demo: `<div class="field" style="width:280px"><label>태그</label><div class="input" style="height:auto;min-height:40px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;padding:6px 12px"><span class="chip" style="height:24px">법무</span><span class="chip" style="height:24px">계약</span><span style="color:var(--gray-9);font-size:var(--text-sm)">입력…</span></div></div>`,
  vue: `<DsCombobox v-model="tags" label="태그" :items="suggested" />`,
  html: null,
  guidelines: [['해야 할 것','목록의 값만 허용해야 하면 Autocomplete를 씁니다.']],
},
{
  id: 'popover', name: 'Popover', ko: '팝오버', category: 'overlay',
  origin: 'wrapped', vuetifyBase: 'VMenu',
  summary: '클릭으로 여는 내용 패널.',
  reason: { ko: 'Tooltip보다 길고 Dialog보다 가벼운 정보가 있습니다. 포지셔닝은 VMenu가 처리합니다.', en: 'Between tooltip and dialog; VMenu does positioning.' },
  props: [['location','string',`'bottom start'`,'열리는 방향.'],['width','number | string','280','패널 폭.'],['variant',`'prose' | 'list'`,`'prose'`,'담는 내용. 설명·짧은 폼이면 prose, 선택지 목록이면 list(메뉴와 같은 4px 인셋).']],
  slots: [['activator','여는 요소. v-bind 필수.'],['default','패널 내용.']],
  demo: `<div class="ds-popover" style="width:240px;box-shadow:none"><b style="color:var(--gray-12);font-size:var(--text-sm)">보존 기간</b><div style="margin-top:6px">180일이 지난 기록은 보관 저장소로 이동합니다.</div></div>`,
  vue: `<DsPopover :width="260">
  <template #activator="props">
    <DsIconButton label="설명" v-bind="props"><DsIcon name="info" /></DsIconButton>
  </template>
  180일이 지난 기록은 보관 저장소로 이동합니다.
</DsPopover>`,
  html: null,
  guidelines: [['하지 말 것','확인이 필요한 입력은 Popover가 아니라 Dialog.'],['하지 말 것','내용을 패딩 있는 래퍼로 다시 감싸지 않습니다. 패널이 인셋을 이미 갖고 있어 두 배가 됩니다. 목록을 담을 땐 variant="list".']],
},
{
  id: 'hovercard', name: 'HoverCard', ko: '호버 카드', category: 'overlay',
  origin: 'wrapped', vuetifyBase: 'VMenu',
  summary: '올리면 뜨는 미리보기.',
  reason: { ko: '지연·포지셔닝·유지 판정을 직접 만들면 깜빡입니다.', en: 'Delay and positioning flicker if hand-rolled.' },
  props: [['location','string',`'bottom start'`,'방향.'],['openDelay','number','350','뜨기까지 ms.']],
  slots: [['activator','대상 요소.'],['default','미리보기 내용.']],
  demo: `<div class="ds-popover ds-hovercard" style="box-shadow:none"><div style="display:flex;gap:10px;align-items:center"><span class="ds-avatar">JK</span><div><b style="color:var(--gray-12);font-size:var(--text-sm)">Jiyong Kim</b><div style="font-size:var(--text-xs);color:var(--gray-10)">편집자 · 법무팀</div></div></div></div>`,
  vue: `<DsHoverCard>
  <template #activator="props">
    <span v-bind="props" class="ds-link">Jiyong Kim</span>
  </template>
  <DsAvatar>JK</DsAvatar> 편집자 · 법무팀
</DsHoverCard>`,
  html: null,
...
export const VUETIFY_COVERAGE = [
  ...
  ['VSlider', 'themed', '', 'thumb 16 · track 6 · 흰 면 + 브랜드 테두리'],
  ['VRangeSlider', 'themed', '', 'thumb 16 · track 6 · 흰 면 + 브랜드 테두리'],
...
export const WHERE = {
  ...
  calendar: '예약 화면, 늘 펼쳐 두는 기간 필터 패널.',
  datepicker: '폼 안의 날짜 한 칸 — 계약 만료일, 감사 로그 기간 필터.',
...
Object.assign(A11Y, {
  ...
  badge: {
    keys: [],
    free: ['상태 색(배경·글자·테두리) + 텍스트를 함께 렌더'],
    yours: ['텍스트를 반드시 넣기 (색만으로 구분하지 않기)', '상태가 바뀌면 aria-live로 알릴지 검토'],
  },
  skeleton: {
    keys: [],
    free: [],
  ...
  calendar: {
    keys: [['Tab', '달력 진입'], ['← → ↑ ↓', '날짜 이동'], ['PageUp · PageDown', '월 이동'], ['Enter', '선택']],
    free: ['Vuetify가 role="grid" · 요일 헤더 · 날짜 읽기 문자열 처리'],
    yours: ['createVuetify에 locale ko와 date ko-KR을 함께 설정 — 하나만 넣으면 절반만 번역됩니다',
            '기간 선택이면 시작·종료를 각각 라벨링'],
  },
  datepicker: {
    keys: [['Enter · Space', '달력 열기'], ['← → ↑ ↓', '날짜 이동'], ['PageUp · PageDown', '월 이동'], ['Enter', '선택'], ['Esc', '닫기 · 필드로 복귀']],
  ...
  link: { keys: [['Tab','링크로 이동'],['Enter','이동']],
    free: ['포커스 링','external 시 rel=noopener'],
    yours: ['링크 텍스트에 목적지를 — "여기" 금지'] },
  ...
  searchfield: { keys: [['Enter','검색 실행'],['Esc','브라우저 기본 — 값 지움']],
    free: ['type=search','지우기 버튼 aria-label'],
    yours: ['결과 수를 aria-live로 알리기'] },
  systemmessage: { keys: [], free: ['role="status"'],
    yours: ['에러는 여기 말고 Alert로'] },
  toolbar: { keys: [['Tab','컨트롤 순회']], free: ['role="toolbar"'],
    yours: ['컨트롤은 전부 sm — 높이 혼재 금지'] },
  visuallyhidden: { keys: [], free: ['표준 sr-only 클립'],
    yours: ['aria-label로 충분한 경우엔 쓰지 않기'] },
  numberinput: { keys: [['↑ ↓','스텝'],['Home/End','최소·최대']],
    free: ['Vuetify가 role=spinbutton·클램프 처리'],
    yours: ['label 필수 · 단위는 hint에'] },
  combobox: { keys: [['글자','입력·검색'],['Enter','칩 확정'],['Backspace','마지막 칩 제거'],['↑ ↓','제안 이동']],
    free: ['IME 조합·칩 관리·combobox 시맨틱'],
    yours: ['허용 형식 안내 (이메일 등)'] },
  popover: { keys: [['Enter/Space','열기'],['Esc','닫기'],['Tab','내용 순회']],
    free: ['포지셔닝·바깥 클릭 닫기·포커스 복귀'],
    yours: ['activator에 뜻 있는 라벨'] },
  hovercard: { keys: [['Tab 포커스','호버와 동일하게 열림']],
    free: ['지연·유지 판정'],
    yours: ['필수 정보 넣지 않기 — 터치에서 안 뜸'] },
...
export const VERSUS = {
  ...
  snackbar: [['Toast', '같은 생김새입니다 — Snackbar가 Toast를 그대로 그립니다. 띄우고 없애는 동작이 필요하면 Snackbar'],
             ['Alert', '지나가도 되면 Snackbar, 남아야 하면 Alert']],
  toast: [['Snackbar', '알림의 생김새는 Toast 하나뿐입니다. 실제로 떠서 사라져야 하면 Snackbar가 이걸 감쌉니다']],
  ...
  link: [['Button', '이동이면 Link, 실행이면 Button']],
  popover: [['Tooltip', '한 줄이면 Tooltip'], ['Dialog', '확인·입력이 필요하면 Dialog']],
  hovercard: [['Popover', '클릭으로 여는 건 Popover']],
